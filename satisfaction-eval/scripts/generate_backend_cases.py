"""Generate additional cases for backend mock_data.py to reach 500 total."""
...
if __name__ == "__main__":
    ...
    lines = []
    for i in range(33, 501):  # C033 to C500
        ...
        line = f'    {{"id": "C{i:03d}", "title": "{loc}{cat}典型案例", "category": "{cat}", "tags": {json.dumps(tags, ensure_ascii=False)}, "problem": "{random_value(problem_tpl)}", "solution": "{random_value(solution)}", "effect": "{random_value(effect)}", "region": "{region}", "createTime": "{date.strftime("%Y-%m-%d")}"}},'
        lines.append(line)
    ...
    # Replace: keep first 32 cases, then insert new cases before the closing bracket
    # Find the end of case_items (line with ']' after C032)
    old_end = '    {"id": "C032", "title": "700MHz频段清频与5G重耕"'
    closing_bracket = "\n]\n\ndata_sources"

    # Find the position
    idx = content.find(old_end)
    if idx >= 0:
        # Find the closing bracket after C032
        c032_end = content.find("\n]", idx)
        if c032_end >= 0:
            new_content = content[:c032_end] + ",\n" + "\n".join(lines) + content[c032_end:]
            with open("backend/app/services/mock_data.py", "w", encoding="utf-8") as f:
                f.write(new_content)
            print(f"Successfully inserted {len(lines)} cases into backend mock_data.py")
        else:
            print("Could not find closing bracket")
    else:
        print("Could not find C032")
